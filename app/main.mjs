...
const app={repo:new Repository(),settings:{...defaults},profile:{id:'default',name:'Traveler',description:'A traveler at the crossroads',pronouns:'',rating:'sfw'},view:'home',session:null,saves:[],assetCache:new Map(),libraryItems:[],libraryQuery:'',libraryGenre:'',librarySort:'name',libraryPage:0,historyLimit:45,draft:'',mobilePanel:null,navEpoch:0,status:{phase:'ready',label:'Saved locally'},stream:'',
  ...
  async begin(story,characters,profile,options){this.session?.stop();const compiled=compile(story),key=await this.repo.freeze(story);const save=createAdventure(compiled,characters,profile,{...options,definitionKey:key});await this.repo.commitSave(save,null);await this.repo.put('meta','activeSave',save.id);await this.attach(save,compiled);await this.navigate('adventure');this.scheduleBackup?.();if(this.settings.voiceAuto)this.speak(save.events[0]?.narration).catch(onError);},
  async attach(save,compiled){this.session?.stop();const provider=await this.provider();this.session=new Session({save,compiled,repository:this.repo,provider,onAutoWait:async()=>{if(this.settings.voiceAuto){await this.voiceTask;await voice.finished();}},onChange:next=>{this.stream='';if(this.view==='adventure')this.render({scrollEnd:true});this.refreshSaves().catch(onError);this.scheduleBackup?.();if(this.settings.voiceAuto)this.voiceTask=this.speak(next.events.at(-1)?.narration || '').catch(onError);},onStatus:status=>{this.status=status;if(status.phase==='generating'){this.stream='';}this.paintStatus();},onChunk:chunk=>{this.stream+=chunk;const el=$('#stream-text');if(el){el.textContent=this.stream;const thread=$('#thread');if(thread && thread.scrollHeight-thread.scrollTop-thread.clientHeight<220)thread.scrollTop=thread.scrollHeight;}}});const stop=this.session.stop.bind(this.session);this.session.stop=()=>{this.stopVoice();stop();};await this.refreshAssets();this.stream='';this.status={phase:'ready',label:'Saved locally'};},
  ...
  stopVoice(){this.voiceEpoch=(this.voiceEpoch || 0)+1;voice.stop();},
  async speak(text){if(!text)return;const epoch=this.voiceEpoch || 0;const {ensureVoicePlayer}=await import('./ui/voice-controls.mjs');if(epoch!==(this.voiceEpoch || 0))return;ensureVoicePlayer();voice.read(text,this.settings);},
...
window.addEventListener('pagehide',()=>{app.session?.stop();app.stopVoice();});
async function boot(){
