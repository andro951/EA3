...
export function field(label,name,value='',{type='text',placeholder='',help='',required=false,rows=3,options=null,extra=''}={}) {
  const input=options?`<select aria-label="${esc(label)}" name="${esc(name)}" ${extra}>${options.map(o=>{const [v,l]=Array.isArray(o)?o:[o,o];return `<option value="${esc(v)}" ${String(v)===String(value)?'selected':''}>${esc(l)}</option>`;}).join('')}</select>`:type==='textarea'?`<textarea aria-label="${esc(label)}" name="${esc(name)}" rows="${rows}" placeholder="${esc(placeholder)}" ${required?'required':''} ${extra}>${esc(value)}</textarea>`:`<input aria-label="${esc(label)}" name="${esc(name)}" type="${esc(type)}" value="${esc(value)}" placeholder="${esc(placeholder)}" ${required?'required':''} ${extra}>`;
