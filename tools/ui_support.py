...
def start_page(browser, native=False, url='http://127.0.0.1:4173', width=1440, height=1000):
    ...
    if native:
        ...
        page.evaluate('''art=>{
          ...
          Object.defineProperty(crypto,'subtle',{value:{digest:async(_alg,data)=>new Uint8Array(await __digest(Array.from(new Uint8Array(data.buffer||data,data.byteOffset||0,data.byteLength)))).buffer},configurable:true});
